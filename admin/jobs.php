...
function fetchJobsApi($url, $label) {
    $raw = curl_load($url);
    if (empty($raw)) return ['ok' => false, 'label' => $label, 'data' => []];
    $data = json_decode($raw, true);
    if (!is_array($data)) return ['ok' => false, 'label' => $label, 'data' => []];
...
<div class="jobs-page">
    <div class="container">
        <div class="jobs-header">
            <h1><i class="fa fa-briefcase"></i> <?= _e('Job Search') ?></h1>
            <p><?= _e('Search workplaces, companies, institutions...') ?></p>
        </div>
        ...
        <form action="" method="get" class="jobs-search">
            <div class="jobs-search-box">
                <input type="text" name="q" value="<?= htmlspecialchars($searchQuery, ENT_QUOTES, 'UTF-8') ?>"
                       placeholder="<?= _e('Search workplaces, companies, institutions...') ?>"
...
<?php if ($remoteDetail): ?>
        <div class="detail-panel">
            <a href="javascript:history.back()" class="detail-back"><i class="fa fa-arrow-right"></i> <?= _e('Back') ?></a>
            ...
            <div class="detail-header" style="text-align:right;">
                ...
                <div style="text-align:center;margin-bottom:16px;">
                    ...
                    <?php endif; ?>
                    <?php $statusLabel = ((int)($remoteDetail['status'] ?? 0) === 1) ? _e('Active') : _e('Inactive'); ?>
                ...
                <div class="detail-stats" style="justify-content:center;margin-bottom:18px;">
                    <div class="detail-stat">
                        ...
                        <div class="lbl"><?= _e('Clients Count') ?></div>
                    </div>
                </div>
                ...
                <div class="detail-section-title"><i class="fa fa-map-marker-alt"></i> <?= _e('Address') ?></div>
                <div class="detail-info-grid">
                    <?php if ($addrCity): ?>
                        <div class="detail-info-item"><b><?= _e('City') ?></b> <?= htmlspecialchars($addrCity) ?></div>
                    <?php endif; ?>
                    <?php if ($addrArea): ?>
                        <div class="detail-info-item"><b><?= _e('Area') ?></b> <?= htmlspecialchars($addrArea) ?></div>
                    <?php endif; ?>
                    <?php if ($addrStreet): ?>
                        <div class="detail-info-item"><b><?= _e('Street') ?></b> <?= htmlspecialchars($addrStreet) ?></div>
                    <?php endif; ?>
                    <?php if ($addrBuilding): ?>
                        <div class="detail-info-item"><b><?= _e('Building') ?></b> <?= htmlspecialchars($addrBuilding) ?></div>
                    <?php endif; ?>
                    <?php if ($addr && !$addrCity && !$addrArea): ?>
                        <div class="detail-info-item" style="grid-column:1/-1"><b><?= _e('Full Address') ?></b> <?= htmlspecialchars($addr) ?></div>
                    <?php endif; ?>
                    <?php if ($postalCode): ?>
                        <div class="detail-info-item"><b><?= _e('Postal Code') ?></b> <?= htmlspecialchars($postalCode) ?></div>
                    <?php endif; ?>
                    <?php if ($plusCode): ?>
                        <div class="detail-info-item"><b><?= _e('Plus Code') ?></b> <?= htmlspecialchars($plusCode) ?></div>
                    <?php endif; ?>
                    <?php if ($mapUrl): ?>
                        <div class="detail-info-item"><b><?= _e('Location') ?></b> <a href="<?= htmlspecialchars($mapUrl) ?>" target="_blank"><i class="fa fa-map-marker-alt"></i> <?= _e('View on Map') ?></a></div>
                    <?php elseif ($lat && $lng): ?>
                        <div class="detail-info-item"><b><?= _e('Location') ?></b> <a href="https://www.google.com/maps?q=<?= $lat ?>,<?= $lng ?>" target="_blank"><i class="fa fa-map-marker-alt"></i> <?= _e('View on Map') ?></a></div>
                    <?php endif; ?>
                </div>
                <?php endif; ?>

                <?php if ($hasContact): ?>
                <div class="detail-section-title" style="margin-top:18px;"><i class="fa fa-address-book"></i> <?= _e('Contact Info') ?></div>
                <div class="detail-info-grid">
                    <?php if ($email): ?>
                        <div class="detail-info-item"><b><?= _e('Email') ?></b> <a href="mailto:<?= htmlspecialchars($email) ?>"><?= htmlspecialchars($email) ?></a></div>
                    <?php endif; ?>
                    <?php if ($website): ?>
                        <div class="detail-info-item"><b><?= _e('Website') ?></b> <a href="<?= htmlspecialchars($website) ?>" target="_blank"><?= htmlspecialchars($website) ?></a></div>
                    <?php endif; ?>
                </div>
                ...
                <div class="detail-section-title" style="margin-top:18px;"><i class="fa fa-clock"></i> <?= _e('Working Hours') ?></div>
                    ...
                    <table>
                        ...
                        <?php foreach ($workingHours as $wh): ?>
                            <tr<?= ((int)($wh['closed'] ?? 0) === 1) ? ' style="opacity:0.4;"' : '' ?>>
                                ...
                                <td><?= ((int)($wh['closed'] ?? 0) === 1) ? '<span style="color:#fca5a5;">' . _e('Closed') . '</span>' : '<span style="color:#86efac;">' . _e('Open') . '</span>' ?></td>
                            </tr>
                        <?php endforeach; ?>
                        </tbody>
                    </table>
                ...
                <?php if ($notes): ?>
                <div class="detail-section-title" style="margin-top:18px;"><i class="fa fa-sticky-note"></i> <?= _e('Notes') ?></div>
...
<?php elseif (!empty($selectedWork)): ?>
        <div class="detail-panel">
            ...
            <div class="section-title"><i class="fa fa-users"></i> <?= _e('Clients') ?> (<?= count($workClients) ?>)</div>
                ...
                <div class="client-row-body">
                    ...
                    <div class="client-row-info">
                        <?php if (!empty($cl['national_id'])): ?>
                            <span><b><?= _e('National ID') ?>:</b> <?= htmlspecialchars($cl['national_id']) ?></span>
                        <?php endif; ?>
                        <?php if (!empty($cl['phone'])): ?>
                            <span dir="ltr"><b><?= _e('Phone') ?>:</b> <?= htmlspecialchars($cl['phone']) ?></span>
                        <?php endif; ?>
                        <?php if (!empty($cl['status'])): ?>
                            <span><b><?= _e('Status') ?>:</b> <?= htmlspecialchars($cl['status']) ?></span>
                        <?php endif; ?>
                    </div>
                </div>
...
<?php elseif (!empty($workplaces) || !empty($remoteWorkplaces)): ?>
        <?php if (!empty($remoteErrors)): ?>
            <div class="remote-warning-jobs">
                <i class="fa fa-exclamation-triangle"></i>
                <?= _e('Warning') ?>: <?= _e('Could not connect to') ?> <b><?= htmlspecialchars(implode('، ', $remoteErrors)) ?></b>
            </div>
        <?php endif; ?>

        <div class="section-title" style="max-width:900px;margin:0 auto 14px;">
            <i class="fa fa-list-ul"></i> <?= _e('Search results for') ?>: <b><?= htmlspecialchars($searchQuery, ENT_QUOTES, 'UTF-8') ?></b>
        ...
        <div class="work-grid">
            ...
            <a href="?work=<?= urlencode($wp['workplace']) ?>" class="work-card">
                ...
                <div class="work-card-meta">
                    <span><i class="fa fa-users"></i> <b><?= (int)$wp['client_count'] ?></b> <?= _e('Clients') ?></span>
                </div>
            ...
            <a href="?work=<?= urlencode($rw['name']) ?>&src=<?= urlencode($rw['_source']) ?>&wid=<?= urlencode($rw['id']) ?>" class="work-card">
                ...
                <div class="work-card-meta">
                    <span><i class="fa fa-users"></i> <b><?= (int)($rw['customers_count'] ?? 0) ?></b> <?= _e('Clients') ?></span>
                </div>
...
<?php elseif (!empty($searchQuery)): ?>
        ...
        <div class="no-results-jobs">
            ...
            <p><?= _e('No workplaces found') ?></p>
        </div>

<?php elseif (!empty($topWorkplaces)): ?>
        <div class="section-title" style="max-width:900px;margin:0 auto 14px;">
            <i class="fa fa-chart-bar"></i> <?= _e('Top Workplaces') ?>
        </div>
        <div class="work-grid">
