...
class Xcrud
{
    ...
    public static function get_requested_instance()
    {
            ...
            $key)
        {
            ...
            self::error('<strong>حدث خطأ في تحميل البيانات</strong><br />
                الرجاء التأكد من عدم فتح صفحات مختلفة بنفس الوقت. وقم باعادة تحميل الصفحة.</strong>');
    }
    ...
    {
        exit('<div class="xcrud-error" style="position:relative;line-height:1.25;padding:15px;color:#BA0303;margin:10px;border:1px solid #BA0303;border-radius:4px;font-family:inherit;background:#FFB5B5;box-shadow:inset 0 0 80px #E58989;">
    ...
    protected function render_grid_body($row_tag = array('tag' => 'tr'), $item = array('tag' => 'td'))
    {
        ...
        if ($this->result_list)
        {
            foreach ($this->result_list as $key => $row)
            {
                ...
                $out .= $this->open_tag($row_tag, 'row-id-' . $row['primary_key'] . ' xcrud-row xcrud-row-' . $i);
                if ($this->is_numbers)
                {
